...
public sealed class CancelPromotionHandler(
    ...
    IEventBus eventBus,
    INotificationPort notificationPort) : IRequestHandler<CancelPromotionCommand, Unit>
{
    public async Task<Unit> Handle(CancelPromotionCommand command, CancellationToken ct)
    {
        ...
        promotion.Cancel(command.UserId);
